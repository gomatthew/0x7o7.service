...
class AgentExecutorAsyncIteratorCallbackHandler(AsyncIteratorCallbackHandler):
    ...
    async def on_chat_model_start(self, serialized: Dict[str, Any], messages: List[List], *, run_id: UUID,
    ...
    async def on_llm_error(self, error: Exception | KeyboardInterrupt, **kwargs: Any) -> None:
        data = {"status": AgentStatus.error, "text": str(error)}
        self.queue.put_nowait(dumps(data))
    ...
    async def on_tool_start(
        ...
        data = {"run_id": str(run_id), "status": AgentStatus.tool_start,
                "tool": serialized["name"], "tool_input": input_str}
        self.queue.put_nowait(dumps(data))

    async def on_tool_end(self, output: str, *, run_id: UUID, parent_run_id: Optional[UUID] = None,
