...
from src.server.ai.rag.kb_service import create_kb, get_kb_list, delete_kb, upload_file_to_kb, upload_text_to_kb, \
    delete_file_to_kb, get_file_seg_list, get_file_progress, rag_retrieve, get_file_list, get_kb_detail

rag_router = APIRouter(prefix='/rag', tags=['rag'], dependencies=[Depends(token_identify), Depends(ai_rate_limit)])
rag_router.post('/create_kb')(create_kb)
rag_router.get('/get_kb_list')(get_kb_list)
rag_router.get('/get_kb_detail')(get_kb_detail)
